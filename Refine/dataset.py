...
def collate_fn(config, data, tokenizer):
    ...
    labels = tokenizer(list(labels), max_length=max_length,
                       truncation=True)["input_ids"]

    inputs = tokenizer(list(texts), return_tensors='pt', padding='max_length',
    ...
    if config['is_causal_lm']:
        for i in range(len(labels)):
            temp = []
            j = 1
            while inputs['input_ids'][i][j+1] != other_token and \
                    inputs['input_ids'][i][j+1] != you_token:
                temp += [-100]
                j += 1
